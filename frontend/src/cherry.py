...
class Root(object):
	...
	def upload(self, searchable):
		...
		if extension != '.mp4':
			...
			HANDLER.runTask(priority=1, onComplete=self.indexAndTranscodeComplete, target=self.transcodeAndIndexUpload, args=(destination, newdestination, searchable, filename, vidHash), kwargs = {'name' : vidHash}, onCompleteArgs=(destination, newdestination, vidHash))
		else:
			HANDLER.runTask(priority=0, onComplete=self.indexComplete, target=self.indexUpload, args=(searchable, filename, vidHash), kwargs = {'name' : vidHash}, onCompleteArgs=(vidHash))
	...
	def transcodeAndIndexUpload(self, destination, newdestination, searchable, filename, vidHash):
		...
		HANDLER.runTask(priority=0, onComplete=self.indexComplete, target=self.indexUpload, args=(searchable, filename, vidHash), kwargs ={'name' : vidHash})
